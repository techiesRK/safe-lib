...
public class ArcProgress extends View {
    ...
    public void setProgress(float progress) {
        this.progress = Float.valueOf((new DecimalFormat("#.##")).format((double)progress));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float finishedSweepAngle = this.progress / (float)this.getMax() * this.arcAngle;
        ...
        if (this.arcBottomHeight == 0.0F) {
            ...
            this.arcBottomHeight = bottomTextBaseline * (float)(1.0 - Math.cos((double)(angle / 180.0F) * Math.PI));
        }
